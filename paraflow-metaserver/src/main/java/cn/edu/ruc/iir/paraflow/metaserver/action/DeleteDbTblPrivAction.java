...
import cn.edu.ruc.iir.paraflow.commons.exceptions.TblPrivDeleteException;
...
public class DeleteDbTblPrivAction extends Action
{
    @Override
    public ActionResponse act(ActionResponse input, Connection connection) throws ParaFlowException
    {
        ...
        if (stringListOp.isPresent() && sizeOp.isPresent()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                String sqlStatement = SQLTemplate.findTblPriv(tblId);
                ResultList resultList = connection.executeQuery(sqlStatement);
                if (!resultList.isEmpty()) {
                    //result
                    String sqlStatement2 = SQLTemplate.deleteTblPriv(tblId);
                    int status = connection.executeUpdate(sqlStatement2);
                    if (status == 0) {
                        throw new TblPrivDeleteException();
                    }
                }
            }
        }
