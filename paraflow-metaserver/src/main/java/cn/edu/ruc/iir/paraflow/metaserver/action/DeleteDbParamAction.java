...
import cn.edu.ruc.iir.paraflow.commons.exceptions.DbParamDeleteException;
 ...
 */
public class DeleteDbParamAction extends Action
{
    @Override
    public ActionResponse act(ActionResponse input, Connection connection) throws ParaFlowException
    {
        ...
        if (dbIdOp.isPresent()) {
            ...
            String sqlStatement = SQLTemplate.findDbParamKey(dbId);
            ResultList resultList = connection.executeQuery(sqlStatement);
            if (!resultList.isEmpty()) {
                //result
                String sqlStatement2 = SQLTemplate.deleteDbParam(dbId);
                int status = connection.executeUpdate(sqlStatement2);
                if (status == 0) {
                    throw new DbParamDeleteException();
                }
            }
        }
        else {
