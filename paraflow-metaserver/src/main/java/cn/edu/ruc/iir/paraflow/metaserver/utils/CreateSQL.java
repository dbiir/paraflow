...
public final class CreateSQL
{
    ...
    public static String createVerModelSql = "CREATE TABLE vermodel (vername varchar(50),constraint unique_ver unique(vername));";
    ...
    public static String createTblModelSql = "CREATE TABLE tblmodel (tblid SERIAL primary key,dbid int REFERENCES dbmodel(dbid),tblname varchar(50),tbltype int,userid int REFERENCES usermodel(userid),createtime bigint,lastaccesstime bigint,locationUrl varchar(100),storageformatid int REFERENCES storageformatmodel(storageformatid),fiberColId int,fiberfuncid int REFERENCES fiberfuncmodel(fiberfuncid),constraint unique_tbl unique(dbid,tblname));";
    public static String createColModelSql = "CREATE TABLE colmodel (colid SERIAL primary key,colIndex int,dbid int REFERENCES dbmodel(dbid),tblid int REFERENCES tblmodel(tblid),colName varchar(50),colType varchar(50),dataType varchar(50),constraint unique_col unique(colIndex,tblid,colName),constraint unique_col2 unique(tblid,colName));";
